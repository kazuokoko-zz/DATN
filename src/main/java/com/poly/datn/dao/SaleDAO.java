...
public interface SaleDAO extends JpaRepository<Sale, Integer> {
    ...
    @Query(nativeQuery = true,value = "select * from sale s where :currtime after s.start_time and s.end_time")
    List<Sale> findSalesAboutStart(@Param("currtime") Timestamp time);
    @Query(nativeQuery = true,value = "select * from sale s where :currtime before s.start_time and s.end_time")
