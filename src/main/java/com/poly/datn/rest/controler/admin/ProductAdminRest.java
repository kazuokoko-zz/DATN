...
public class ProductAdminRest {
    ...
    public ResponseEntity<ResponseDTO<Object>> newProductDetail(@RequestBody List<ProductDetailsVO> productDetailsVOS, Principal principal){
        return ResponseEntity.ok(ResponseDTO.builder().object(productDetailService.newProductDetail(productDetailsVOS, principal)).code(Constant.RESPONSEDTO_CODE).message(Constant.RESPONSEDTO_MESS).build());
    }
    @PostMapping("newproductcolor")
    public ResponseEntity<ResponseDTO<Object>> newProductColor(@RequestBody List<ProductColorVO> productColorVOS, Principal principal){
        return ResponseEntity.ok(ResponseDTO.builder().object(productColorService.newProductColor(productColorVOS, principal)).code(Constant.RESPONSEDTO_CODE).message(Constant.RESPONSEDTO_MESS).build());
    }
}
